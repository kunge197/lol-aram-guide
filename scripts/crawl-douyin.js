...
const BUILD_PARSE_PROMPT = `你是一个英雄联盟海克斯大乱斗套路分析师。从以下抖音视频文案中提取套路信息。文案可能来自语音转录，会有错别字或不通顺，但请尽力从中提取有效信息。

核心任务：**必须识别出英雄是谁**，这是最高优先级。

英雄识别规则：
1. **#标签最高优先级** — 视频标题/描述中的 #英雄名（如 #蔚、#金克斯、#亚索）是识别英雄的最可靠依据
2. **文案推理** — 如果视频#标签被截断或不全，从文案内容推断英雄（如提到"E技能"、"Q技能"、英雄特性描述、技能名称等）
3. **必须尽力猜测** — 即使文案不完整，也要根据你掌握的英雄联盟知识推断可能的英雄，不要随意填 null
...
套路提取规则：
- 即使文案不完整，也尽力推断装备和符文
- 如果实在推断不出具体装备，items 可以填空数组 []
- buildTitle 永远不要返回 null——即使不确定，也要根据内容想一个描述性名称（如"爆发法伤流"、"坦克抗伤流"等）
- 如果文案明显不是游戏内容（如美食、旅游、日常生活），则 items 填 []，buildTitle 填 "无关内容"

只返回 JSON 格式（不要 markdown 代码块）：
{
  "champion": "英雄中文名/常用称呼，无法确定填 null",
  "championEn": "英雄英文名首字母大写，如 Yasuo、Jinx、Brand，无法确定填 null",
  "buildTitle": "套路名称，永远不要返回 null",
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (process.env.DOUYIN_DEBUG) {
        log(`  [DEBUG] 文案前 200 字: ${(transcript || "").slice(0, 200)}`);
        log(`  [DEBUG] LLM 原始返回: ${content.slice(0, 500)}`);
      }
